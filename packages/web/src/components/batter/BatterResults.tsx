...
export default function BatterResults() {
  const batter = usePitchStore((s) => s.batter);
  const startBatterReview = usePitchStore((s) => s.startBatterReview);
  const exitBatterMode = usePitchStore((s) => s.exitBatterMode);

  const stats = computeAccuracy(batter.calls);
