...
export default function UmpireReviewHUD() {
  const umpire = usePitchStore((s) => s.umpire);
  const pitches = usePitchStore((s) => s.pitches);
  const reviewNext = usePitchStore((s) => s.reviewNext);
  const reviewPrev = usePitchStore((s) => s.reviewPrev);
  const exitUmpireMode = usePitchStore((s) => s.exitUmpireMode);

  const pitchId = umpire.pitchQueue[umpire.reviewIndex];
  const pitch = pitches.find((p) => p.id === pitchId);
  const call = umpire.calls[umpire.reviewIndex];

  const isFirst = umpire.reviewIndex === 0;
  const isLast = umpire.reviewIndex === umpire.pitchQueue.length - 1;

  const handleReplay = () => {
    window.dispatchEvent(new CustomEvent("umpire-replay"));
  };
      ...
      <div className="flex items-center justify-between">
        <div className="bg-zinc-900/80 backdrop-blur rounded-lg px-4 py-2 pointer-events-auto">
          <span className="text-sm font-medium text-zinc-100">
            Review: Pitch {umpire.reviewIndex + 1} of{" "}
            {umpire.pitchQueue.length}
          </span>
        </div>
        <button
